...
export class PRDetailService {
  async fetchPRDetail(
    ...
    const raw = await this.fetchRawPRDetail(repoOwnerName, prNumber);
    const reviewIds = raw.reviews.map((r) => r.id);

    const inlineCommentsByReview = await this.fetchReviewInlineComments(
      repoOwnerName,
      prNumber,
      reviewIds
    );
  ...
  private async fetchRawPRDetail(
    repoOwnerName: string,
    prNumber: number
  ): Promise<GhPRDetail> {
    try {
      ...
      return JSON.parse(stdout) as GhPRDetail;
    } catch (error) {
      throw mapGhError(error, 'fetch');
    }
  }

  private async fetchReviewInlineComments(
    repoOwnerName: string,
    prNumber: number,
    reviewIds: string[]
  ): Promise<Map<string, GhReviewInlineComment[]>> {
    const [allComments, nodeIdToNumericId] = await Promise.all([
      this.fetchRawPRInlineComments(repoOwnerName, prNumber),
      this.resolveNodeIdToNumericId(repoOwnerName, prNumber, reviewIds),
    ]);
  ...
  private async fetchRawPRInlineComments(
    repoOwnerName: string,
    prNumber: number
  ...
  private async resolveNodeIdToNumericId(
    repoOwnerName: string,
    prNumber: number,
    reviewIds: string[]
